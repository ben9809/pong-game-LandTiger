...
void LCD_border_init(void) {
	
	int i;
	int color = Red;
	unsigned int end_line = 320;
	
	for(i = 0; i < 5; i++) {
		LCD_DrawLine(i, 0, i, end_line, color);
		//LCD_DrawLine(i, i, MAX_X-i, i, color);
		LCD_DrawLine(MAX_X - i - 1, 0, MAX_X - i - 1, end_line, color);
	}
}
